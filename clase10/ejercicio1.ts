...
Luego, simula la preparación de tres pedidos (por ejemplo: Pizza, Hamburguesa y Ensalada) con tiempos de preparación distintos. 
La función debe imprimir un mensaje al iniciar cada pedido y otro mensaje cuando el pedido esté listo. Además:
...
function prepararPedido (plato: string, tiempo: number, callback: any) {
    console.log(`Preparando ${plato}...`) //La función debe imprimir un mensaje al iniciar cada pedido
    
    setTimeout( () => {
        console.log(`${plato} está listo`)//otro mensaje cuando el pedido esté listo.
        callback()
    } , tiempo)
}

function completarTodosLosPedidos() {
    console.log("¡Todos los pedidos han sido completados!"); //Implementa otro callback que se ejecute al final, después de todos los pedidos, para mostrar el mensaje: "¡Todos los pedidos han sido completados!".
}

prepararPedido('Hamburguesa', 5000, ()=> {
    prepararPedido('Papas fritas', 3000, () => {
        prepararPedido ('Pizzas', 2000, () => {
             completarTodosLosPedidos()})
    })
})